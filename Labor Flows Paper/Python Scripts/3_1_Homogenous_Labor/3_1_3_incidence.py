...
This is an extension of 3.1.2 using differing static parameters
...
## Note that Moretti references the labor elasticity. For now, I'll ignore this
...
Further, following Moretti, assume the two cities are identifical in period 1. Additionally:
    - set k_a/k_b equal (except in 3 and 4) 
    - set idiosyncratic preferences relatively equal
    - set Ammeinities equal as well

Change the following for each scenario:

1) Imobile labor. Set location preferences such that no migration occurs
    - benefit of the shock accrues entirely to workers in city b
    - Land prices remain unchanged

2) Labor is perfectly mobile. Set s=0 for all locations 

For 1 and 2, run a sweep of some additional runs of s between 0 and inf - possibly could do this, save for later though
...
## Set base seed
seed = 202207
write_path = r'C:\Users\zachm\Documents\George Mason\Projects\Labor Flows Paper\Results\3_1_Homogenous_Labor\3_1_3_incidence'
...
"""
World 3: Housing supply in b is fixed (kb=inf)

Note that if we set the value too high relative to the number of workers in the simulation, we end up with numerical problems that mess the model up in unexpected ways
"""
for ii in range(runs):   
    ...
    ## Add Locations
    world.addLocation(housing_elasticity = 10, local_ammenities = np.log(4), productivity_shifter = {'Sector 1': 6}, location_ID = "Location A")
    world.addLocation(housing_elasticity = 10, local_ammenities = np.log(4), productivity_shifter = {'Sector 1': 6}, location_ID = "Location B")
    ...
    # stats_labor.to_csv(write_path + r'\Case_3' + r'\labor_results_' + str(ii) + '.csv') # These are pretty big, let's avoid writing them...
...
"""
World 4: Housing supply in b is infinitely elastic (kb=0)
"""
for ii in range(runs):   
    ...
    ## Add Locations
    world.addLocation(housing_elasticity = 0, local_ammenities = np.log(4), productivity_shifter = {'Sector 1': 6}, location_ID = "Location A")
    world.addLocation(housing_elasticity = 0, local_ammenities = np.log(4), productivity_shifter = {'Sector 1': 6}, location_ID = "Location B")
    ...
    # stats_labor.to_csv(write_path + r'\Case_4' + r'\labor_results_' + str(ii) + '.csv') # These are pretty big, let's avoid writing them...
