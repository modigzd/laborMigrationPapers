# -*- coding: utf-8 -*-
"""
Replicate Section 3.2.2: Effect of a Labor Demand Shock on Wages and Prices
                         for Heterogenous Labor (2 sectors)

Here, run to equilibrium, then shock a location for skilled workers only:
    - Increase X_c 
    - Increase X_ic as an extension (potentially) <- actually, provide technology to each firm individually from an exponential distribution
...
## Set base seed
seed = 202207
write_path = r'C:\Users\zachm\Documents\George Mason\Projects\Labor Flows Paper\Results\3_2_Heterogenous_Labor\3_2_2_with_firm_technology'

start = time.time()

runs = 3

n_steps = 200 # How many steps should be in the model pre and post shock?
"""
Case 1: Low k, Even number of skilled/unskilled workers
"""
for ii in range(runs):   
    ...
    ## Add Locations
    world.addLocation(housing_elasticity = 1, local_ammenities = np.log(4), productivity_shifter = {'skilled': 6, 'unskilled': 6}, location_ID = "Location A")
    world.addLocation(housing_elasticity = 1, local_ammenities = np.log(4), productivity_shifter = {'skilled': 6, 'unskilled': 6}, location_ID = "Location B")
    ...
    for f in world.Firms.values():
        ...
        f.productivity_shift = np.random.exponential(scale = 1, size = 1)[0]
    ...
    stats_loc, stats_firm, stats_labor, stats_network = world.simulate(steps = n_steps,
    ...
    s_stats_loc, s_stats_firm, s_stats_labor, s_stats_network = world.simulate(steps = n_steps, is_shock = True, time_offset = n_steps,
                                                                              alpha = labor_search, beta = firm_search)
