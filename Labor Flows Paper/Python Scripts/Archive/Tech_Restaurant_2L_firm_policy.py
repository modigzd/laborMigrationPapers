...
Here, we only allow the smallest 10 firms to offer remote work arrangments 
within the tech sector to see how it impacts outcomes
...
## Set base seed
seed = 202111
write_path = r'C:\Users\zachm\Documents\George Mason\Projects\Labor Flows Paper\Results\Tech_Restaurant_2L_firm_policy'
write_csv = False
...
for ii in range(50):
    ...
    ## Update remote tolerances for workers in the tech sector, the smallest 10
    n_empl = []
    f_name = []
    for f in test.Firms.values():
        ...
        if f.sector == 'Tech':
            ...
            n_empl.append(len(f.labor))
            f_name.append(f.name)
        
        
    ## Sort the list and take the smallest 10 firms
    idx = np.argsort(n_empl)
    for jj in idx[0:9]:
        
        new_tol = np.random.normal(.9, .075)
        new_tol = min(max(0, new_tol), 1) 
        test.Firms[f_name[jj]].updateRemotePref(new_tol)
